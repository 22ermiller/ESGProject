...
price_annuity <- function(
  ...
  if (start_age * 12 >= min(mortality_tbl$month) - 1) {
    S_base <- mortality_tbl[mortality_tbl$month == start_age * 12 - 1,]$S
...
get_death_ages <- function(start_age, tbl, n) {
  
  filtered_mortality <- tbl %>%
    filter(month > start_age * 12) %>%
    mutate(death_pdf = death_pdf / sum(death_pdf))
  
  death_ages <- sample(
    filtered_mortality$month,
    size = n,
    replace = TRUE,
    prob = filtered_mortality$death_pdf
  )
