...
interest_df <- read_csv("data/interest_rate.csv") |>
  rename(date = observation_date, rate = DGS3MO) |>
...
full_df <- sp500 |>
  ...
  mutate(
    sp500_rminterest = log_dif_sp500 - rate / 12,
    sp500_rminterest_rmmean = sp500_rminterest -
      mean(sp500_rminterest, na.rm = TRUE)
  )
...
full_garch_spec <- ugarchspec(
  variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
  mean.model = list(armaOrder = c(0, 0), external.regressors = X),
  distribution.model = "norm"
)
...
cpi_garch_spec <- ugarchspec(
  variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
  mean.model = list(
    armaOrder = c(0, 0),
    external.regressors = X_cpi,
    include.mean = FALSE
  ),
  distribution.model = "norm"
)
...
saveRDS(
  list(spec = spec, data = mean_mod_resids),
  "models/equity_rs_mod_specs.rds"
)
...
cpi_simulate <- function(
  model,
  n = 600,
  start_date = max(full_df$date) + months(1),
  final_val = tail(full_df$cpi, 1)
) {
  sim <- simulate(model, nsim = n)
  sim_df <- data.frame(
    date = seq(from = start_date, by = "month", length.out = n),
    cpi_sim = sim
  )
  final_df <- sim_df %>%
...
sp500_rs_simulate <- function(
  ...
  n = 600,
  start_date = max(full_df$date) + months(1),
  final_value = tail(full_df$close, 1),
  og_mean = mean(full_df$sp500_rminterest),
  response_type = "log_sp500",
  cpi_type = "log_dif_cpi"
) {
  ...
  final_df <- sim_df %>%
    mutate(
      temp = mean_vals + resid_sim,
      realsp500 = case_when(
        response_type == "log_sp500" ~ final_value * exp(cumsum(temp)),
        response_type == "sp500_rminterest" ~ final_value *
          exp(cumsum(temp + log(1 + interest_sim))),
...
sp500_sim <- sp500_rs_simulate(
  mean_mod,
  mean_mod1_rs,
  cpi_sim$cpi_sim,
  interest_sim$real_rate,
  n = 600
)
sp500_rminterest_sim <- sp500_rs_simulate(
  mean_mod2,
  mean_mod2_rs,
  cpi_sim$cpi_sim,
  interest_sim$real_rate,
...
sp500_rminterest_rmmean_sim <- sp500_rs_simulate(
  mean_mod3,
  mean_mod3_rs,
  cpi_sim$cpi_sim,
  interest_sim$real_rate,
...
cpi_sp500_sim <- sp500_rs_simulate(
  cpi_mean_mod,
  cpi_mean_mod1_rs,
  cpi_sim$real_cpi,
  interest_sim$real_rate,
...
cpi_sp500_rminterest_sim <- sp500_rs_simulate(
  cpi_mean_mod2,
  cpi_mean_mod2_rs,
  cpi_sim$real_cpi,
  interest_sim$real_rate,
...
cpi_sp500_rminterest_rmmean_sim <- sp500_rs_simulate(
  cpi_mean_mod3,
  cpi_mean_mod3_rs,
  cpi_sim$real_cpi,
  interest_sim$real_rate,
...
for (i in 1:n_sims) {
  ...
  interest_sim <- interest_simulate(
    interest_mod,
    cpi_sim$cpi_sim,
    n = sim_length
  )

  sp500_sim[i, ] <- sp500_rs_simulate(
    mean_mod,
    mean_mod1_rs,
    cpi_sim$cpi_sim,
    interest_sim$real_rate,
  ...
  sp500_rminterest_sim[i, ] <- sp500_rs_simulate(
    mean_mod2,
    mean_mod2_rs,
    cpi_sim$cpi_sim,
    interest_sim$real_rate,
  ...
  sp500_rminterest_rmmean_sim[i, ] <- sp500_rs_simulate(
    mean_mod3,
    mean_mod3_rs,
    cpi_sim$cpi_sim,
    interest_sim$real_rate,
...
sp500_rminterest_ci <- apply(
  sp500_rminterest_sim,
  2,
  quantile,
  probs = c(.025, .975)
)
sp500_rminterest_rmmean_ci <- apply(
  sp500_rminterest_rmmean_sim,
  2,
  quantile,
  probs = c(.025, .975)
)
cpi_sp500_ci <- apply(cpi_sp500_sim, 2, quantile, probs = c(.025, .975))
cpi_sp500_rminterest_ci <- apply(
  cpi_sp500_rminterest_sim,
  2,
  quantile,
  probs = c(.025, .975)
)
cpi_sp500_rminterest_rmmean_ci <- apply(
  cpi_sp500_rminterest_rmmean_sim,
  2,
  quantile,
  probs = c(.025, .975)
)
...
plot_sims <- function(avg, ci, title = "Simulation") {
  plot(avg, type = "l", ylim = c(-.2, .2), main = title)
