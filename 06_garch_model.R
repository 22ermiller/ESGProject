...
transformed_rate_ts <- tk_ts(
  model_df$transformed_rate,
  start = c(min(year(model_df$date)), month(min(model_df$date))),
  frequency = 12
)

# fit AR(1,0,1) model

transformed_ar1 <- Arima(
  transformed_rate_ts,
  order = c(1, 0, 1),
...
transformed_auto_ar <- auto.arima(
  transformed_rate_ts,
  xreg = cpi,
  method = "ML"
)
...
dif_fgarch_fit <- garchFit(
  formula = transformed_dif ~ arma(1, 1) + garch(1, 1),
  data = model_df,
  cond.dist = "norm"
)
...
single_interest_simulation <- function(sim_length = 20 * 12) {
  # AR101 simulation
  ar_sim <- apply(
    as.matrix(simulate(transformed_ar1, nsim = sim_length)),
    1,
    back_transform
  )

  # interest rate w/out cpi simulation
  simple_garch_sim <- apply(
    as.matrix(
      ugarchsim(
        simple_garch_fit,
        n.sim = sim_length,
  ...
  garch_sim <- apply(as.matrix(sim_results), 1, back_transform)

  dif_garch_sim <- ugarchsim(
    dif_garch_fit,
    n.sim = sim_length,
    startMethod = "sample",
    mexsimdata = mexsim_list
...
multiple_interest_simulation <- function(n_sims = 1000, sim_length = 80 * 12) {
  ...
  for (i in 1:n_sims) {
    ...
    sim_results <- ugarchsim(
      ...
      mean(model_df$transformed_rate)
    garch_sims[i, ] <- apply(as.matrix(sim_results), 1, back_transform)
  }
  #
...
single_inflation_simulation <- function(sim_length = 20 * 12) {
  ...
  lagged_cpi_sim <- c(
    tail(quarterly_cpi$log_dif_cpi, n = 1),
  ...
  mtext(
    paste("Correlation:", round(simulation_cor, 3)),
    outer = TRUE,
    cex = 1.2,
    line = -1
  )
