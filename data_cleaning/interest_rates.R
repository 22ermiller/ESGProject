...
ir3mo_raw <- read_csv("raw_data/DGS3MO.csv")
...
ir2yr_raw <- read_csv("raw_data/DGS2.csv")
...
ir3mo <- ir3mo_raw |>
       ...
       filter(
              date >= min(cpi_df$date) - months(1) & date <= max(cpi_df$date)
       ) |>
       dplyr::select(date, rate)

ir10yr <- ir10yr_raw |>
       rename(date = observation_date, rate = DGS10) |>
       filter(!is.na(rate)) |>
       mutate(year = year(date), month = month(date), day = day(date)) |>
       group_by(year, month) |>
       arrange(day) |>
...
ir30yr <- ir30yr_raw |>
       rename(date = observation_date, rate = DGS30) |>
       filter(!is.na(rate)) |>
       mutate(year = year(date), month = month(date), day = day(date)) |>
       group_by(year, month) |>
       arrange(day) |>
...
ir2yr <- ir2yr_raw |>
       rename(date = observation_date, rate = DGS2) |>
       filter(!is.na(rate)) |>
       mutate(year = year(date), month = month(date), day = day(date)) |>
       group_by(year, month) |>
       arrange(day) |>
...
ir3yr <- ir3yr_raw |>
       rename(date = observation_date, rate = DGS3) |>
       filter(!is.na(rate)) |>
       mutate(year = year(date), month = month(date), day = day(date)) |>
       group_by(year, month) |>
       arrange(day) |>
...
ir5yr <- ir5yr_raw |>
       rename(date = observation_date, rate = DGS5) |>
       filter(!is.na(rate)) |>
       mutate(year = year(date), month = month(date), day = day(date)) |>
       group_by(year, month) |>
       arrange(day) |>
...
ir7yr <- ir7yr_raw |>
       rename(date = observation_date, rate = DGS7) |>
       filter(!is.na(rate)) |>
       mutate(year = year(date), month = month(date), day = day(date)) |>
       group_by(year, month) |>
       arrange(day) |>
...
ir20yr <- ir20yr_raw |>
       rename(date = observation_date, rate = DGS20) |>
       filter(!is.na(rate)) |>
       mutate(year = year(date), month = month(date), day = day(date)) |>
       group_by(year, month) |>
       arrange(day) |>
